# Spring Boot Backend Engineer — Junie Guidelines

These guidelines define how Junie builds, extends, and tests feature-based
Spring Boot REST APIs. Use them when adding a new feature/endpoint, a new
entity + CRUD, or refactoring an existing
controller/service/repository/DTO/mapper to match the project's layered,
feature-packaged conventions. Junie owns its own tests and SonarQube quality —
write unit/integration/controller tests following the project's documented
patterns and resolve Sonar issues.

Apply the conventions below exactly. This document overrides existing code
where they disagree — when you touch legacy code on the old pattern, migrate
it; change only what the task needs and state what changed and why.
...
- **You own your tests.** After every feature/change, write unit/integration/controller tests following `.junie/docs/*` exactly, then `mvn verify` and resolve SonarQube issues. [Testing & quality]
...
2. **Inspect `src/main/java`** to find the **actual base package** and the
   existing feature packages and naming style already in use.
...
| Tests | you write them — patterns in `.junie/docs/*` |
...
`.junie/docs/*` files are the **single source of truth** for how tests are
...
1. **Prerequisites** — read `.junie/docs/INITIAL_TEST_PREQUISITES.md`. Verify
...
3. **Write tests following the matching doc exactly:**
   - Unit (service isolation): `.junie/docs/UNIT_TESTING.md`
   - Integration (HTTP + DB + WireMock): `.junie/docs/INTEGRATION_TESTING.md`
   - Controller (web layer / validation): `.junie/docs/CONTROLLER_TESTING.md`
...
5. **SonarQube** — when a Sonar setup exists, resolve issues following
   `.junie/docs/SONARQUBE.md` exactly: never `@SuppressWarnings`, exclusions go
...
12. **Tests** for everything touched, following `.junie/docs/*` exactly.
    [Testing & quality]
13. **Validate**: `mvn verify` green; SonarQube issues resolved per
    `.junie/docs/SONARQUBE.md`. [Testing & quality]
